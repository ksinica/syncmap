...
func (c mapCall) apply(m mapInterface[string, string]) (any, bool) {
	switch c.op {
	case opLoad:
		return m.Load(c.k)
	case opStore:
	...
	case opCompareAndSwap:
		if m.CompareAndSwap(c.k, c.v, randValue(rand.New(rand.NewSource(0)))) {
			m.Delete(c.k)
			return c.v, true
		}
		return nil, false
...
func applyMap(calls []mapCall) ([]mapResult, map[string]string) {
	return applyCalls(new(syncmap.Map[string, string]), calls)
}

func applyRWMutexMap(calls []mapCall) ([]mapResult, map[string]string) {
	return applyCalls(new(RWMutexMap[string, string]), calls)
}

func applyDeepCopyMap(calls []mapCall) ([]mapResult, map[string]string) {
