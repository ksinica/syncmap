...
type bench[K, V comparable] struct {
	setup func(*testing.B, mapInterface[K, V])
	perG  func(b *testing.B, pb *testing.PB, i int, m mapInterface[K, V])
}

func benchMap[K, V comparable](b *testing.B, bench bench[K, V]) {
	for _, m := range [...]mapInterface[K, V]{&DeepCopyMap[K, V]{}, &RWMutexMap[K, V]{}, &syncmap.Map[K, V]{}} {
